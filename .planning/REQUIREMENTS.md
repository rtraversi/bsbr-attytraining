...
- [ ] **RENEW-03**: Renewal reuses the SAME single volume-tiered Stripe Price ID (**`price_1U6BAj5md3Gcv1Z13Rx9qQll`** on live account `acct_1ThDpU5md3Gcv1Z1` — *corrected 2026-08-27. This requirement previously named the sandbox `price_1TjNHc6ZCSojEKRrKs79ToJ0` and said no lookup_key existed, so `per_seat_annual` could not be used as a handle. The live Price **does** carry `lookup_key: per_seat_annual`, and `lib/stripe-price.ts` resolves by that key rather than by ID*) and the firm's current seat quantity (flat annual pricing — no renewal discount); the renewal charges the same per-seat band rate as year one. There is exactly one Price — no separate discounted renewal price.
