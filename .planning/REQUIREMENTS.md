...
- [ ] **RENEW-03**: Renewal reuses the SAME single volume-tiered Stripe Price ID (`price_1TjNHc6ZCSojEKRrKs79ToJ0` — *corrected 2026-08-03; this requirement previously named `price_1ThbLNCzT2268ei9nkadS8kD`, an object on a retired Stripe account. The real price carries **no** lookup_key, so `per_seat_annual` cannot be used as a handle*) and the firm's current seat quantity (flat annual pricing — no renewal discount); the renewal charges the same per-seat band rate as year one. There is exactly one Price — no separate discounted renewal price.
