...
3. **Re-auth wrangler** to the right Cloudflare account (§1). Does not block shipping, but blocks
   local inspection and the documented `wrangler rollback`. **Still true on 2026-08-27** — the
   OAuth session is on `2809122619f6ccd79e32edeb6e98504c` while the Workers live in
   `4b2a402334decc9259d7317aaf9782f0` (`DEPLOY-RUNBOOK.md:52`), so `wrangler secret list` returns
   `Authentication error [code: 10000]`. The **Cloudflare MCP is on the right account** and can list
   the Workers, so it is a usable stand-in for read-only checks — but note it has **no secrets
   tool**, and `wrangler secret list` returns names only, so **neither can ever reveal a secret
   value.** Do not try to verify which Stripe key prod holds this way; see `CLAUDE.md` §4.
4. ✅ **Stripe live mode — DONE, and it has been for longer than the docs said.** Verified against
   the Stripe API 2026-08-27. Live account `acct_1ThDpU5md3Gcv1Z1`, charges and payouts enabled.
   Stripe Tax **is** active (head office Garner NC, NC registration
   `taxreg_1U6B2M5md3Gcv1Z1r6EWlpt2`), so `automatic_tax` does not hard-fail. Live Product
   `prod_V6NwTwWVBDkz7R` — **already renamed** to "Iurix Accreditation — Annual Certification" —
   and live Price `price_1U6BAj5md3Gcv1Z13Rx9qQll` with `lookup_key: per_seat_annual`,
   `tax_behavior: exclusive` and `tax_code: txcd_20060058`. All three are set; `ix-stripeaudit` is
   closed. **A real card was charged $37.54 on 2026-08-19**, refunded, subscription cancelled and
   the prod firm it created purged on 2026-08-27.
   **Still open, all dashboard, none blocking:** move the payout bank account to the LLC's; set the
   statement descriptor back to `IURIX ACCREDITATION` (the entity change made it
   `BSBR HOLDINGS LLC`, which customers will not recognise on a statement).
   🔴 **The one real Stripe blocker is now `app/api/webhooks/stripe/route.ts:630`** — it emails a
   non-US buyer that their payment is being refunded, and `refunds.create` appears nowhere in the
   codebase. That was harmless on sandbox money. It is not now.
...
> **Corrected 2026-08-27 (Rob).** The "existing EIN — no new entity, no new EIN" part was wrong.
> **BSBR Holdings, LLC was newly approved and a new EIN was issued**, both landing 2026-08-27. It is
> a **multi-member LLC — Rob and Katy**, which is why Stripe's ownership requirements applied.
> Registered on the Stripe account the same day; requirements completed. This also resolves **open
> question #3 in `.planning/legal/README.md`** — the entity is "BSBR Holdings, LLC d/b/a Iurix",
> Iurix is not becoming its own LLC, and the seven places in the app and legal docs already say so.
>
> ⚠️ **Do not verify entity state via `GET /v1/account`.** On a standard non-Connect account it
> still returns `business_type: individual`, `company.name: "Robert M Traversi"` and
> `requirements: null` regardless of the truth. Legacy data. The dashboard's **Account status** tab
> is the only authority — this was misdiagnosed twice on 2026-08-27. "Built Smart by Rob" is a sibling brand and is removed
